...
class E:  # Secp256k1 # !!! WARNING ! THIS WALLET GENERATOR IS EXPERIMENTAL !!!
    ...
    def __rmul__(P,x,Q=None):    # 79 x 35 #║──╣─╔═══╗─║─ wallet.py ── v1.9 ──║
    ...
    def __radd__(P,Q):  # Vires in Numeris #║github.com/exander77/simplecrypto║
        ...
        x=(s**2-d)%P.p;return E(x,(s*(P.x-x)-P.y)%P.p,P.p)
...
W=lambda p:M(p+h(h(p))[:4]);J=lambda d:J(d//32)+[d%32] if d>0 else [];A+='JKLM'
...
N=lambda d:[0]+N(d[1:]) if d[0]==0 else J(int.from_bytes(d,B));B='big'
def L(s,A='qpzry9x8gf2tvdw0s3jn54khce6mua7l',h='bc',w='1',R=range(6)):
    t=[0]+N(list(g(s)));u=O(H(h)+t+[0]*6)^1
    c=[(u>>5*(5-i))&0x1f for i in R]; return h+w+''.join([A[d] for d in t+c])
if len(i)!=64: S.exit('Usage: python3'+T*3%(S.argv[0],'<'*3,h(b'').hex()))
a=bytes.fromhex(i);B='big';d=int.from_bytes(a,B);C='Private key';D='Address'
P=E(y=0xc0c686408d517dfd67c2367651380d00d126e4229631fd03f8ff35eef1a61e3c,
    x=0x3b78ce563f89a0ed9414f5aa28ad0d96d6795f9c63,p=2**256-2**32-977)*2*d
Y=b'\x80'+a;print(U%(C+' (HEX)',i,C+' (WIF XY)',W(Y),C+' (WIF X)',W(Y+b'\x01'),
    D+' (XY)',K(P.toXY()),D+' (X)',K(P.toX()),D+' (SegWit Bech32)',L(P.toX())))
