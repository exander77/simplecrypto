import hashwal,sys as S;N=int.from_bytes;B='big';i=input();_=32;C='Private key'
class E: # Secp256k1 # Vires in Numeris. #╔═══════════════════════════════════╗
    ...
    def __radd__(P,Q):  # Funds are SAFU. ║ github.com/exander77/simplecrypto ║
        ...
        x=(s**2-d)%P.p;return E(x,(s*(P.x-x)-P.y)%P.p,P.p) # Time for Plan B ?!
    def __mul__(P,x,Q=None):[(x&1<<i and(Q:=Q+P),P:=P+P)for i in P.R];return Q#
    def __init__(P,x,y,p=2**256-2**_-977):P.x=x;P.y=y;P.p=p;P.R=range(256) # LU
    def X(P):return(b'\x03'if P.y%2else b'\x02')+P.x.to_bytes(_,B)# The Times #
...
q=lambda V,G,c:q(V[1:],G,l(c>>25,0,G,(c&2**25-1)<<5^V[0]))if len(V)else c #2FNI
W=lambda p,A=A+'bcdefghijkmnopqrstuvwxyz':M(J(p+h(h(p))[:4],58),A);w='bc' #4KNI 
...
a=bytes.fromhex(i);d=N(a,B)# Long live Satoshi Nakamoto. Remember Hal Finney. #
P=E(y=0xc0c686408d517dfd67c2367651380d00d126e4229631fd03f8ff35eef1a61e3c, # BTC
    x=0x3b78ce563f89a0ed9414f5aa28ad0d96d6795f9c63)*2*d;X=b'\x80'+a;S=' (WIF X'
D='Address';print(('%32s : %s\n'*6)[:-1]%(C+' (HEX)',i,C+S+'Y)',W(X),C+S+')',W(
    X+b'\x01'),D+' (XY)',K(P.Y()),D+' (X)',K(P.X()),D+' (Bech32)',L(P.X()))) #.
