...
class ECC: # Secp256k1: https://en.bitcoin.it/wiki/Secp256k1
    ...
    def __rmul__(P, x, Q=None):            #║──╣─╔═══╗─║─ wallet.py ── v1.3 ──║
...
def b2w(q, p): return q+g(int.from_bytes(p+h(h(p))[:4], B), '')
def p2a(s): return b2w('1', b'\x00'+h(h(s), 'ripemd160'))
...
print(U%('%s (HEX)'%V, a, '%s (WIF XY)'%V, b2w('', Y+b), '%s (WIF X)'%V, b2w(''
    , Y+b+X), '%s (XY)'%W, p2a(P.toXY()), '%s (X)'%W, p2a(P.toX())))     # QED.
